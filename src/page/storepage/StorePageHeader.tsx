...
import { FilterListChangeEventProps, FilterListOperatorType, FilterListProps, FilterProps, FilterSearchChangeEventProps, FilterSearchProps, FilterToggleChangeEventProps, FilterToggleProps, FilterTreeChangeEventProps, FilterTreeProps } from "../../data/filters/common";
...
function StorePageHeader({ countApi, filters, onFilterChange, status, statusApi, title }: StorePageHeaderProps) {
    const
        [qs, setQS] = useSearchParams(),
        [navFilter, setNavFilter] = React.useState<FilterTreeProps | null>(null),
        [statusFilter, setStatusFilter] = React.useState<FilterToggleProps | null>(null),
        ...
        [searchFilter, setSearchFilter] = React.useState<FilterSearchProps | null>(null),
        [filterValue, setFilterValue] = useRecoilState(FilterValue),
        [count, setCount] = React.useState<number | boolean>(false),
        [statusResult, setStatusResult] = React.useState<StatusChipProps | undefined>(status),
        { t } = useTranslation("nui");

    React.useEffect(() => {
        let
            visible: (FilterListProps | FilterListProps)[] = [],
        ...
        filters && filters.forEach(f => {
            switch (f.type) {
                case "list":
                    ...
                    if (listQS) {
                        let listQSArray = listQS.split("|"),
                            op = listQSArray.shift() as FilterListOperatorType;

                        listValue = {
                            operator: op,
                            selected: listQSArray.map(s => (
                                parseInt(s) || s
                            ))
                        };
                    }
                    ...
                    if (listValue) {
                        visible.push({
                            ...
                            ...{
                                value: listValue,
                                onChange: ((e: FilterListChangeEventProps) => {
                                    const changeListQS = [e.value.operator, ...e.value.selected].join("|");
                ...
                case "toggle":
                    ...
                    if (toggleQS) {
                        toggleValue = toggleQS.split("|").map(t => (
                            parseInt(t) || t
                        ));
                    }
        ...
        >
            ...
            >
                <Typography
                    ...
                    variant="h5"
                >
                    {`${title} ${count === false ? "" : `(${count.toLocaleString()})`}`}
                </Typography>
                {
                    statusResult &&
                    <StatusChip
            ...
            {
                (statusFilter || hiddenFilters.length || visibleFilters.length || searchFilter) &&
                <Stack
                    direction="row"
                    spacing={2}
                    sx={{ width: 1 }}
